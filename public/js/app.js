...
// REAL inter-exchange backbone landlines from the GÉANT pan-European feed
// (map.geant.org). Served by /api/backbone-links and drawn into the Fiber
// Backbone overlay (toggle-fiber) as actual city-to-city backbone links plus
// their POP nodes — replacing the earlier synthetic hub-and-spoke guesses.
async function loadIXBackbone() {
  if (!layers.fiberBackbone) layers.fiberBackbone = L.layerGroup().addTo(map);
  try {
    const data = await fetchJSON('/api/backbone-links');
    const links = (data && data.links) || [];
    const nodes = (data && data.nodes) || [];

    links.forEach(link => {
      if (!link.coords || link.coords.length < 2) return;
      L.polyline(link.coords, { color: '#00e0a8', weight: 2.5, opacity: 0.75 })
        .bindPopup(`
          <div class="popup-title">🔗 ${link.name}</div>
          <div class="popup-meta">
            🌍 GÉANT pan-European backbone${link.capacity ? '<br>⚡ ' + link.capacity : ''}
          </div>
          <span class="popup-badge" style="background:#00e0a8;color:#000">BACKBONE LANDLINE</span>
        `)
    ...
    nodes.forEach(n => {
      L.circleMarker([n.lat, n.lng], {
        radius: 3, color: '#00e0a8', weight: 1, fillColor: '#00e0a8', fillOpacity: 0.9
      })
        .bindPopup(`<div class="popup-title">${n.name}</div><div class="popup-meta">GÉANT POP${n.open_exchange ? ' · Open Exchange' : ''}</div><span class="popup-badge" style="background:#00e0a8;color:#000">BACKBONE NODE</span>`)
        .addTo(layers.fiberBackbone);
    });

    console.log(`Loaded ${links.length} GÉANT backbone landlines, ${nodes.length} POP nodes`);
  } catch (err) {
    console.error('Failed to load backbone landlines:', err);
  }
}
